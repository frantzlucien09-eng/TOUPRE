-- Add delivery proof photo support

-- 1. Add delivery_proof_url column to orders
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS delivery_proof_url text;

-- 2. Create delivery-proofs storage bucket (public read so customer can view proof)
INSERT INTO storage.buckets (id, name, public)
VALUES ('delivery-proofs', 'delivery-proofs', true)
ON CONFLICT (id) DO NOTHING;

-- 3. Storage policies for delivery-proofs bucket
DROP POLICY IF EXISTS "authed_upload_delivery_proofs" ON storage.objects;
CREATE POLICY "authed_upload_delivery_proofs" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'delivery-proofs');

DROP POLICY IF EXISTS "public_read_delivery_proofs" ON storage.objects;
CREATE POLICY "public_read_delivery_proofs" ON storage.objects
  FOR SELECT USING (bucket_id = 'delivery-proofs');

-- 4. Update update_order_status RPC to accept optional delivery proof URL
...
BEGIN
  ...
  -- Update the order (include delivery proof if provided)
