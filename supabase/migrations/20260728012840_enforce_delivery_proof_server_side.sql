-- Enforce delivery proof server-side: reject 'delivered' without proof for delivery orders
...
BEGIN
  ...
  -- Server-side enforcement: delivery orders must have proof photo for 'delivered'
  IF p_new_status = 'delivered' AND COALESCE(p_delivery_proof_url, v_order.delivery_proof_url) IS NULL AND v_order.delivery_method = 'delivery' THEN
